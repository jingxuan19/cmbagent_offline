...
# Ollama patch
def _extract_plan_from_chat(cmbagent_instance):
    """
    Extract plan from chat history when function calling fails to record it.
    Returns (plan_list, num_steps) or (None, None) if parsing fails.
    """
    import re
    
    if not hasattr(cmbagent_instance, 'chat_result') or not cmbagent_instance.chat_result:
        return None, None
    
    chat_history = getattr(cmbagent_instance.chat_result, 'chat_history', [])
    
    for msg in reversed(chat_history):
        content_str = msg.get('content', '') if isinstance(msg, dict) else str(msg)
        
        # Look for formatted plan output
        if '**PLAN**' not in content_str and '- Step 1:' not in content_str:
            continue
        ...
        for line in content_str.split('\n'):
            ...
            if re.match(r'^-?\s*Step\s+\d+:', line_stripped, re.IGNORECASE):
                if current_step and current_step.get('sub_task'):
                    steps.append(current_step)
                current_step = {'sub_task': '', 'sub_task_agent': 'engineer', 'bullet_points': []}
                in_instructions = False
                continue
            ...
            # Match sub-task
            if '* sub-task:' in line_stripped.lower():
                current_step['sub_task'] = line_stripped.split(':', 1)[1].strip()
            # Match agent
            elif '* agent in charge:' in line_stripped.lower() or '* agent:' in line_stripped.lower():
                agent = line_stripped.split(':', 1)[1].strip().lower()
                if 'engineer' in agent:
                    current_step['sub_task_agent'] = 'engineer'
                elif 'research' in agent:
                    current_step['sub_task_agent'] = 'researcher'
                else:
                    current_step['sub_task_agent'] = agent
            # Match instructions header
            elif '* instructions:' in line_stripped.lower():
                in_instructions = True
            # Match bullet points
            elif line_stripped.startswith('- ') and in_instructions:
                current_step['bullet_points'].append(line_stripped[2:].strip())
        ...
        if steps:
            print(f"[OLLAMA FIX] Parsed {len(steps)} steps from chat history")
...
class CMBAgent:
    ...
    def __init__(self,
        ...
        # GEMINI FIX: Remove unsupported parameters for Google API
        if llm_config_list and llm_config_list[0].get('api_type') == 'google':
            self.llm_config.pop('top_p', None)
            self.llm_config.pop('temperature', None)
        ...
        #     # update llm config with api keys
        #     # I don't think api_keys can be None??? Since it has to be type KeyManager????
        #     if self.llm_config["config_list"][0].get("api_type", None) != "ollama":
        #         # Check if model is Ollama, if not, fill api keys
        ...
        #         if self.agent_llm_configs[agent].get("api_type", None) != "ollama":
...
def planning_and_control_context_carryover(
    ...
    if restart_at_step <= 0:
        ...
        # OLLAMA FIX: If final_plan is None, extract from chat history
        if planning_output.get('final_plan') is None or planning_output.get('number_of_steps_in_plan') is None:
            print("[OLLAMA FIX] Plan not recorded via function call, parsing from chat history...")
            parsed_plan, num_steps = _extract_plan_from_chat(cmbagent)
            if parsed_plan:
                planning_output['final_plan'] = parsed_plan
                planning_output['number_of_steps_in_plan'] = num_steps
                planning_output['plans'] = [parsed_plan]
                planning_output['proposed_plan'] = parsed_plan
                # Also update the cmbagent context for saving
                cmbagent.final_context['final_plan'] = parsed_plan
                cmbagent.final_context['number_of_steps_in_plan'] = num_steps
                print(f"[OLLAMA FIX] Recovered plan with {num_steps} steps!")
            else:
                print("[OLLAMA FIX] WARNING: Could not parse plan from chat history")
    ...
    current_context = copy.deepcopy(planning_output) if restart_at_step <= 0 else load_context(os.path.join(context_dir, f"context_step_{restart_at_step-1}.pkl"))
    number_of_steps_in_plan = current_context.get('number_of_steps_in_plan')
    
    # OLLAMA FIX: If number_of_steps is None, try to recover from final_plan.json
    if number_of_steps_in_plan is None:
        print("WARNING: number_of_steps_in_plan is None, attempting recovery...")
        plan_file = planning_dir / "final_plan.json"
        if plan_file.exists():
            with open(plan_file, 'r') as pf:
                plan_data = json.load(pf)
            if plan_data.get("sub_tasks"):
                number_of_steps_in_plan = len(plan_data["sub_tasks"])
                current_context['number_of_steps_in_plan'] = number_of_steps_in_plan
                print(f"Recovered {number_of_steps_in_plan} steps from final_plan.json")
        
        # If still None, the plan wasn't captured - we need to parse from saved outputs
        if number_of_steps_in_plan is None or number_of_steps_in_plan == 0:
            print("WARNING: No steps in plan. Checking for saved plan text...")
            # Look for any plan-related files
            import glob
            plan_txts = glob.glob(str(planning_dir / "*.txt")) + glob.glob(str(planning_dir / "*.md"))
            print(f"Found files: {plan_txts}")
            
            # Default to 6 steps if we can't find anything (based on typical plan structure)
            if number_of_steps_in_plan is None or number_of_steps_in_plan == 0:
                print("ERROR: Could not determine number of steps. Planning failed to record properly.")
                print("This is likely due to the local model not properly calling recording functions.")
                raise ValueError("Planning phase completed but no plan was recorded. Try using a model with better function calling support.")
